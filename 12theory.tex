...
\begin{definition}
Graph is an ordered pair $G = (V,\ E)$, where $V = \{v_i\}_{i=1}^{n_v}$ is a set of vertices \footnote{\label{note:nodes}Vertices are often called nodes.} and $E = \{e_i\}_{i=0}^{n_e}$ is a set of edges, where edge $e = \{u,\ v\},\ u,\ v \in V$ is an \emph{unordered} pair of vertices.
\end{definition}

\begin{definition}
\label{def:digraph}
Directed graph (digraph) is an ordered pair $G = (V,\ E)$, where $V = \{v_i\}_{i=1}^{n_v}$ is a set of vertices and $E = \{e_i\}_{i=0}^{n_e}$ is a set of edges\footnote{\label{note:arcs}Commonly the edges in directed graphs are called arcs.}, where edge $e = (u,\ v),\ u,\ v \in V$ is an \emph{ordered} pair of vertices.
\end{definition}


\begin{definition}
Multigraph is an ordered tuple $G = (V,\ E,\ r)$, where $V = \{v_i\}_{i=1}^{n_v}$ is a set of vertices, $E = \{e_i\}_{i=0}^{n_e}$ is a set of edges and $r:\ E \rightarrow \{\{u,\ v\}\ |\ u,\ v \in V\}$ is a mapping from edges to pair of endpoint vertices.
\end{definition}

\begin{definition}
Directed multigraph is an ordered tuple $G = (V,\ E,\ s,\ t)$, where $V = \{v_i\}_{i=1}^{n_v}$ is a set of vertices, $E = \{e_i\}_{i=0}^{n_e}$ is a set of edges and $s:\ E \rightarrow V$ is a source (start) mapping from edges to vertices and $t:\ E \rightarrow V$ is a target (end) mapping from edges to vertices.
\end{definition}
...
\begin{definition}
Topological order is a sequence of vertices in directed graph $(v_i)_{i=1}^{|V|}$ such that $e = (u, v) \implies u \prec v$ ($u$ appears before $v$ in the sequence).
\end{definition}
...
\begin{definition}
Alternative graph is an ordered tuple $G = (V,\ E,\ A)$, where $V = \{v_i\}_{i=1}^{n_v}$ is a set of vertices, $E = \{e_i\}_{i=0}^{n_e}$ is a set edges $e = (u,\ v),\ u,\ v \in V$, and $A = \{a_i\}_{i=0}^{n_a}$ is a set of disjunctive (alternative) edge pairs $a = ((u,\ v),\ (x,\ y)),\ u,\ v,\ x,\ y \in V$.
\end{definition}
