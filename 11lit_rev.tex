...
The instances are published with best known solutions (assumed with unlimited time), unfortunatelly for the 10 minute computation time limit only the competition team rankings are presented, without the actual solutions.
...
Referenced by the winning solution is \emph{A Branch and Bound Algorithm for Scheduling Trains in a Railway Network} \cite{DARIANO2007643}. It uses an alternative graph formulation for the train scheduling problem. With this representation we can model resouce conflicts with alternative precedence arcs. It also descibes the idea of inferred selection, meaning that by choosing one precence alternative for a resource implementation selection on another resources (the alternative is forbidden as it would produce an infeasible solution).
